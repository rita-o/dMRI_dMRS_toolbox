...
% clear; clc; close all;
% 
% folder_results   = "Z:\DATA\CSI\9.4T-FIDCSI-Data\20250417_084715_Toi_dMRSI_20250417_RatFem_dMRI_SPECIAL_dMRS_1_1_10_cc";
% expnb       = 35; % scan number

% modif: a_Create_study_Bruker_MouseCryo_9p4T_EM(folder_results , expnb) ->
...
% if ~endsWith(folder_results,"\"); folder_results = folder_results+"\"; end % if missing "\" in directory
if ~endsWith(folder_results,"/") 
    folder_results = folder_results+"/" ;
end % if missing "\" in directory % linux exe
if ~endsWith(folder_exp,"/") 
    folder_exp = folder_exp+"/" ;
end % if missing "\" in directory % linux exe


study.path=char(folder_exp+num2str(expnb));
% methodfile = fileread(folder_results +num2str(expnb)+ "\method");
methodfile = fileread(folder_exp +num2str(expnb)+ "/method"); % linux exe
...
% MB: adapt coil elements according to coil type
if strcmp(coil_type,'rat')
    coil_elements=4;
elseif strcmp(coil_type,'mouse')
    coil_elements=2;
end
...
BRUKERparamnames=["PVM_SpecAcquisitionTime=","##$PVM_SpecSWH=( 1 )", ...
    ...
    ["##$PVM_ArrayPhase=( " + num2str(coil_elements) + " )" + sprintf('\n')],["##$PVM_EncChanScaling=( " + num2str(coil_elements) + " )"+ sprintf('\n')]];% mouse: ##$PVM_ArrayPhase=( 2 ) & "##$PVM_EncChanScaling=( 2 )" /rat:##$PVM_ArrayPhase=( 4 ) &  "##$PVM_EncChanScaling=( 4 )" 
...
% if isfile(folder_exp +num2str(expnb)+ "\acqus")
%     acqusfile = fileread(folder_exp +num2str(expnb)+ "\acqus");
if isfile(folder_exp +num2str(expnb)+ "/acqus")
    acqusfile = fileread(folder_exp +num2str(expnb)+ "/acqus");
...
% 
% elseif isfile(folder_exp +num2str(expnb)+ "\acqp")
%     acqpfile = fileread(folder_exp +num2str(expnb)+ "\acqp");
elseif isfile(folder_exp +num2str(expnb)+ "/acqp")
    acqpfile = fileread(folder_exp +num2str(expnb)+ "/acqp");
...
%job0 as ser/fid
if isfile(folder_exp +num2str(expnb)+ "\ser") % case PV360 v1 where there is a ser file in the main folder (2D acq opened in topspin)
    disp ("2D ser, " + "nav=" + num2str(study.nav) + " nrep=" + num2str(study.nrep) + " saved")
    % fileid=fopen(folder_exp +num2str(expnb)+ "\ser",'r','ieee-le'); %read binary format
        fileid=fopen(folder_exp +num2str(expnb)+ "/ser",'r','ieee-le'); %read binary format
    if fileid == -1
...
% elseif isfile(folder_exp +num2str(expnb)+ "\fid") % case PV360 v1 where there is a fid file in the main folder (2D acq NOT opened in topspin or 1D acq opened or not in topspin)
elseif isfile(folder_exp +num2str(expnb)+ "/fid") % case PV360 v1 where there is a fid file in the main folder (2D acq NOT opened in topspin or 1D acq opened or not in topspin)
     if study.nrep==1
        dimtype="1D";
    else
    ...
    % fileid=fopen(folder_exp +num2str(expnb)+ "\fid",'r','ieee-le'); %read binary format
    fileid=fopen(folder_exp +num2str(expnb)+ "/fid",'r','ieee-le'); %read binary format
    if fileid == -1
        disp('Cannot open file');
        return
    ...
    buffer=fread(fileid,'int32'); %native format v1 for MRS is int32 
% elseif isfile(folder_exp +num2str(expnb)+ "\pv2tsdata\1\ser") % case PV360 v3 where there is a ser file in the subfolder pv2tsdata\1\ (2D acq opened in topspin)
elseif isfile(folder_exp +num2str(expnb)+ "/pv2tsdata/1/ser") % case PV360 v3 where there is a ser file in the subfolder pv2tsdata\1\ (2D acq opened in topspin)
    disp ("2D ser, " + "nav=" + num2str(study.nav) + " nrep=" + num2str(study.nrep) + " saved")
    % fileid=fopen(folder_exp +num2str(expnb)+ "\pv2tsdata\1\ser",'r','ieee-le'); %read binary format
    fileid=fopen(folder_exp +num2str(expnb)+ "/pv2tsdata/1/ser",'r','ieee-le'); %read binary format
    if fileid == -1
        disp('Cannot open file');
        return
...
% elseif  isfile(folder_exp +num2str(expnb)+ "\pv2tsdata\1\fid") % case PV360 v3 where there is a fid file in the subfolder pv2tsdata\1\ (1D acq opened in topspin)
elseif  isfile(folder_exp +num2str(expnb)+ "/pv2tsdata/1/fid") % case PV360 v3 where there is a fid file in the subfolder pv2tsdata\1\ (1D acq opened in topspin)
    disp ("1D fid, " + "nav=" + num2str(study.nav) + " nrep=" + num2str(study.nrep) + " saved")
    % fileid=fopen(folder_exp +num2str(expnb)+ "\pv2tsdata\1\fid",'r','ieee-le'); %read binary format
fileid=fopen(folder_exp +num2str(expnb)+ "/pv2tsdata/1fid",'r','ieee-le'); %read binary format
    if fileid == -1
        disp('Cannot open file');
        return
    end
...
% elseif  isfile(folder_exp +num2str(expnb)+ "\pdata\1\fid_proc.64") % case PV360 v3 where there is a fid file in the subfolder pdata\1\ (1D or 2D acq NOT opened in topspin)
elseif  isfile(folder_exp +num2str(expnb)+ "/pdata/1/fid_proc.64") % case PV360 v3 where there is a fid file in the subfolder pdata\1\ (1D or 2D acq NOT opened in topspin)
    disp ("1D fid, " + "nav=" + num2str(study.nav) + " nrep=" + num2str(study.nrep) + " saved")
    % fileid=fopen(folder_exp +num2str(expnb)+ "\pdata\1\fid_proc.64",'r','ieee-le'); %read binary format
fileid=fopen(folder_exp +num2str(expnb)+ "/pdata/1/fid_proc.64",'r','ieee-le'); %read binary format
    if fileid == -1
        disp('Cannot open file');
        return
...
study.liststring=char(folder_exp + study.filename);
...
% pathrawdata=folder_exp +num2str(expnb)+ "\rawdata.job0";
pathrawdata=folder_exp +num2str(expnb)+ "/rawdata.job0";
...
if isfile(folder_exp + num2str(expnb)+ "\fid.refscan") %for 360v1
    ...
    %store data
    disp('refscan saved')
    % fileid=fopen(folder_exp + num2str(expnb)+ "\fid.refscan",'r','ieee-le'); %read binary format
    fileid=fopen(folder_exp + num2str(expnb)+ "/fid.refscan",'r','ieee-le'); %read binary format
    if fileid == -1
        disp('Cannot open file');
        return
...
% elseif isfile(folder_exp + num2str(expnb)+ "\pdata\1\fid_refscan.64") %for 360v3.3
elseif isfile(folder_exp + num2str(expnb)+ "/pdata/1/fid_refscan.64") %for 360v3.3
...
%%
    ...
    % save(folder_exp + filename,'study');
